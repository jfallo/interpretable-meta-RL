...
def sample_dependent_discrete_hard(batch_size, num_arms, device):
...
def sample_dependent_discrete_medium(batch_size, num_arms, device):
    p1 = torch.where(torch.rand(batch_size, device= device) < 0.5,
...
def sample_dependent_discrete_easy(batch_size, num_arms, device):
    p1 = torch.where(torch.rand(batch_size, device= device) < 0.5,
